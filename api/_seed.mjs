...
export async function fromSeed(view, env = process.env) {
  ...
  const r = await fetch(`${base}/api/owner?org=${encodeURIComponent(org)}&view=${view}`, { headers: { Authorization: `Bearer ${key}` }, cache: "no-store" });
...
export function proxy(view) {
  return async function handler(_req, res) {
    ...
    try {
      res.statusCode = 200;
      res.end(JSON.stringify(await fromSeed(view)));
    } catch (e) {
      res.statusCode = 500;
